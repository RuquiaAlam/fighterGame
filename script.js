...
class Player {
  ...
  strike(player, enemy, attackDamage) {
    ...
    const damageAmount = Math.ceil(Math.random() * attackDamage);
    ...
    return `${player.name} attacks ${enemy.name} for damage ${damageAmount}`;
  }
  ...
  heal(player) {
    ...
    return `Player ${player.name} has ${healAmount}}`;
  }
}
...
const player1 = new Player("ruquia", 100, 10);
const player2 = new Player("Alam", 100, 10);
